...
$perm = $_SESSION['auth'];
 if ($perm == 'admin') { ?>
<li><a href="admin.php">website</a></li>
<?php } ?>
</ul>
</div>
<br/>
...
<form name="web_form" id="web_form" method="post" action="/process-reps.php" enctype="multipart/form-data">
        ...
        Hvis du ønsker dit billede placeret anderledes, kan du selv beskære det kvadratisk, og uploade det.)</label><br><input <?php if (strlen($profile) < 1) { ?> required="required" <?php } ?> type="file" name="fileToUpload" id="fileToUpload"/></p>
